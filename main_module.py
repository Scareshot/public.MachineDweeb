...
#region OUTDATED SETUP CODE; FROM SOME OLD BUILD BUT KEPT INCASE OF NEED FOR A REFERENCE. IGNORE!
'''
import os.path
import discord
import asyncio
...
# needa test commands w/ diff perm levels, and test to see if i can grab channel/user/perms/role/whatever from a command
...
async def on_ready():
    ...
    #await client.change_presence(status=discord.Status, activity=discord.CustomActivity(name='Being Redesigned :) | Version %s' % VERSION))
